...
def get_supplier_by_name(search: str, db:Session):
    try:
        
        supplier_result = db.query(tables.Suppliers).filter(tables.Suppliers.name_supplier.like(f"{search}%")).all()
        if supplier_result:
            return generate_response("success", 200, "search suppliers successfully", supplier_result)
        else:
            raise HTTPException(status_code=404, detail=generate_response("error", 404, "suppliers not found"))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")

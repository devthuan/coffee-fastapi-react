...
def get_order_by_user_id( page: int = Query(1), limit: int = Query(20), db: Session = Depends(get_db), current_user: AuthUser = Depends(validate_token)):
    role_id, list_permission = current_user
    print(role_id)
    if not has_permission(list_permission, "GET_ALL_ORDER", role_id):

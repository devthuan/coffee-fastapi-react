...
def get_all_user(page: int = Query(1, gt=0), limit: int = Query(10, gt=0),db:Session = Depends(get_db), current_user: AuthUser = Depends(validate_token) ):
    try:
        ...
        if not has_permission(list_permission, "GET_ALL_USER", role_id):
...
def update_user(new_user: EditUser, user_id, db: Session = Depends(get_db),current_user: AuthUser = Depends(validate_token) ):
    role_id, list_permission = current_user
    if not has_permission(list_permission, "UPDATE_ROLE_USER", role_id):
            return generate_response("error", status.HTTP_401_UNAUTHORIZED, "You do not have access !")
    try:
...
def change_status_user(user_id, db : Session = Depends(get_db),current_user: AuthUser = Depends(validate_token)):
    role_id, list_permission = current_user
    if not has_permission(list_permission, "DELETE_USER", role_id):
            return generate_response("error", status.HTTP_401_UNAUTHORIZED, "You do not have access !")

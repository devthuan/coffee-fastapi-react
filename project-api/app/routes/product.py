...
from app.services.authenticate import has_permission, validate_token
...
async def create_product(name: Annotated[str, Form()],
    ...
    current_user: AuthUser = Depends(validate_token)):
    role_id, list_permission = current_user
    if not has_permission(list_permission, "CREATE_PRODUCT", role_id):
...
async def update_product(product_id,
    ...
    role_id, list_permission = current_user
    if not has_permission(list_permission, "UPDATE_PRODUCT", role_id):
            return generate_response("error", status.HTTP_401_UNAUTHORIZED, "You do not have access !")
...
def change_status_user(product_id, db : Session = Depends(get_db), current_user: AuthUser = Depends(validate_token)):
    role_id, list_permission = current_user
    if not has_permission(list_permission, "DELETE_PRODUCT", role_id):
            return generate_response("error", status.HTTP_401_UNAUTHORIZED, "You do not have access !")
